      ...
      {open ? (
        ...
        >
          <motion.section
            ...
            className="glass-surface max-h-[88vh] w-full max-w-lg overflow-y-auto rounded-[26px] p-4 sm:p-6"
          >
            ...
            <div className="mb-5 grid grid-cols-3 gap-2">
              {["Resolve key", "Confirm", "Receipt"].map((label, index) => (
                <motion.div
                  ...
                  className={`rounded-2xl px-2 py-2 text-center text-[11px] font-bold sm:text-xs ${step >= index + 1 ? "bg-[#7C3AED]/25 text-white" : "bg-white/[0.04] text-[#8892B0]"}`}
                >
                  {label}
                </motion.div>
              ))}
                  ...
                  <form onSubmit={payForm.handleSubmit(confirmPayment)} className="space-y-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                      ...
                      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
                        ...
                        <p className="mt-2 text-xl font-bold text-[#00FFA3]">{formatMoney(balance)}</p>
                      </div>
                    </div>
                    <label className="block">
                    ...
                    <div className="rounded-2xl border border-white/10 bg-black/20 p-3">
                      ...
                      <p className="mt-2 break-all font-mono text-[11px] text-[#00E5FF]">{idempotencyKey}</p>
                    </div>
                    {errorMessage ? <ErrorBox message={errorMessage} /> : null}
              ...
              {step === 3 ? (
                <StepPanel key="receipt">
                  <div className="py-5 text-center">
                    <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-[#00FFA3]/15">
                      <CheckCircle2 className="h-8 w-8 text-[#00FFA3]" />
                    </motion.div>
                    <h3 className="text-2xl font-black text-white">Transfer committed</h3>
                    <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-[#8892B0]">The receipt PDF was generated by the API and attached to the debit transaction.</p>
                    {receipt?.transactionId ? (
                      <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-3 text-left">
                        <p className="text-xs uppercase tracking-[0.2em] text-[#8892B0]">Receipt transaction</p>
                        <p className="mt-2 break-all font-mono text-xs text-[#00E5FF]">{receipt.transactionId}</p>
                      </div>
                    ) : null}
...
function RecipientCard({ user, paymentKey }: { user: ApiUser; paymentKey: string }) {
  return (
    <div className="rounded-3xl border border-[#00E5FF]/20 bg-[#00E5FF]/[0.06] p-4">
      ...
      <div className="mt-4 grid gap-2 text-sm text-[#AEB8CF] sm:grid-cols-2">
